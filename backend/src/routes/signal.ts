...
function analyzeSignal(signal: number[]) {
	...
	for (let f = 10; f <= 20; f += 0.1) {
		...
		for (let i = 0; i < n; i++) {
			...
			const angle = 2 * Math.PI * f * time[i];
			real += sample * Math.cos(angle);
			imag += sample * Math.sin(angle);
		}

		const amplitude = (2 / n) * Math.sqrt(real * real + imag *imag);
		if (amplitude > maxAmp) {
			maxAmp = amplitude;
			dominantFreq = f;
		}
...
signalRouter.get('/:id', async (req, res) => {
	...
	const result = await pool.query('SELECT signal FROM measures WHERE id = $1', [id]);
